...
export MY_PREFIX="gke-cc-negs"  # just a prefix to help with unique names such as with buckets
...
export CLUSTER_NAME="conf-conn-negs-demo"

export CC_SA_NAME="conf-conn-negs-demo-sa"
export CC_NAMESPACE="conf-conn-negs-demo" 
...
export NEG_NAME=$(kubectl get svc neg-demo-svc -n $CC_NAMESPACE -o jsonpath="{.metadata.annotations.cloud\.google\.com/neg-status}" | jq '.network_endpoint_groups | {name: .["80"]}' | jq .name -r)

# add NEG to backend service for each zone
kubectl get svc neg-demo-svc -n $CC_NAMESPACE -o jsonpath="{.metadata.annotations.cloud\.google\.com/neg-status}" \
